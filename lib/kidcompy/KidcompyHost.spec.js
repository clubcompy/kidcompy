...
describe("KidcompyHost.spec", function() {
  ...
  describe("init", function() {
    it("pulls the id out of theDiv", function() {
      ...
      expect.isNotNull(kidcompyHost);
      expect.isTrue(DomTools.hasClass(theDiv, "kcHost"), "the host div has its css class applied");

      expect.equals(expectedId, kidcompyHost.nodeId, "The id should have been extracted from theDiv");
    });
    ...
    it("adds FlipBuffers to theDiv", function() {
      ...
      expect.equals(2, kidcompyHost.flipBuffers.length, "The flipbuffers were attached");
      expect.equals("f0", kidcompyHost.flipBuffers[0].nodeId);
      expect.equals("f1", kidcompyHost.flipBuffers[1].nodeId);

      // the mutations log should have a record of the DOM appends inside KidcompyHost
      expect.isNotNull(domMutations.findEntryByClassName(DomEventType.APPEND_CHILD, "f0"));
      expect.isNotNull(domMutations.findEntryByClassName(DomEventType.APPEND_CHILD, "f1"));
    });
  ...
  describe("attachFlipBuffers", function() {
    it("constructs two FlipBuffers with expected attributes", function() {
      ...
      expect.equals(2, flipBuffers.length, "there are 2 flipbuffers");
      expect.equals("f0", flipBuffers[0].nodeId, "first id is correct");
      expect.equals("f1", flipBuffers[1].nodeId, "first id is correct");

      expect.equals(2, Array.from(theDiv.childNodes).length, "there are two new child Elements under theDiv");
    });
  });
});

module.exports = "KidcompyHost.spec.js";
