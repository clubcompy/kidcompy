  ...
  iePolyfillInitialize = iePolyfillCheck
    .flatMap(function(isIePolyfillNeeded) {
      ...
      if(isIePolyfillNeeded) {
        ...
        return initFactory().reduce(function() {
          return "iePolyfill initialized";
        });
      }
      else {
        return kidcompy.rxjs.Observable.just("iePolyfill skipped");
      }
    })
    .doOnCompleted(function() {
      kidcompy.log("iePolyfill initialize complete");
    });

  iePolyfillSequence = kidcompy.rxjs.Observable.concat(iePolyfillInject, iePolyfillInitialize)
    .doOnCompleted(function() {
      kidcompy.log("iePolyfill sequence complete");
    });

  iePolyfillSequence
    .reduce(kidcompy.identity, true)
    .forEach(
      function(n) {
