...
describe("FlipBuffer.spec", function() {
  ...
  describe("init", function() {
    it("should add its div to the Host's div", sinon.test(function() {
      ...
      proclaim.isTrue(DomTools.hasClass(rootDiv.firstChild, "kcFlipBuffer"), "the FlipBuffer's div has the css style class name");
      proclaim.isTrue(DomTools.hasClass(rootDiv.firstChild, "f0"), "the FlipBuffer's div has the identifier class name");
      proclaim.equal("f0", flipBuffer.nodeId, "the flipBuffer has a copy of its div's nodeId");
    }));
    ...
    it("will not add 'last' css classname to flipbuffer div's when isLast is false", sinon.test(function() {
      ...
      proclaim.isFalse(DomTools.hasClass(rootDiv.firstChild, "last"), "No 'last' class on " + flipBuffer.nodeId);
    }));
    ...
    it("will add 'last' css classname to flipbuffer div's when isLast is true", sinon.test(function() {
      ...
      proclaim.isTrue(DomTools.hasClass(rootDiv.firstChild, "last"), "'last' class daded on " + flipBuffer.nodeId);
    }));
    ...
    it("must add 8 Playfields to the FlipBuffer", function() {
      ...
      for(var i = 0, ii = 8; i < ii; i++) {
        ...
        proclaim.equal("p" + i, flipBuffer.playfields[i].nodeId, "ID's are correct");
      }
    });
