 ...
 */
Html5ScrollableCanvas.prototype.paintCharacterMap = function(x, y, fillStyle, charMap) {
  // jshint -W016
  ...
  var self = this,
    ...
    i, ii, cx, cy, rx, ry, rw, rh,
    drawList = charMap.drawList;

  if(!drawList.length) {
    return;
  }

  // CONSIDER: cache the fillStyle and set it only if it changes.  Would we need to abstract the context to do this?
  canvasContexts[0].fillStyle = fillStyle;
  canvasContexts[0].beginPath();

  // get the top left coordinates of the character on the target canvas
  cx = x * charMap.pixelWidth;
  cy = y * charMap.pixelHeight;

  // make rectangles in the path for each of the rectangles in our draw list
  for(i = 0, ii = drawList.length; i < ii; i += 4) {
    rx = drawList[i] + cx;
    ry = drawList[i + 1] + cy;
    rw = drawList[i + 2];
    rh = drawList[i + 3];

    canvasContexts[0].rect(rx, ry, rw, rh);
  }
...
exportPublicProperties(Html5ScrollableCanvas.prototype, [
  ...
  ["paintCharacterMap", Html5ScrollableCanvas.prototype.paintCharacterMap]
]);
