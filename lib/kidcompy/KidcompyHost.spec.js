...
describe("KidcompyHost", function() {
  ...
  describe("attachFlipBuffers", function() {
    it("constructs two FlipBuffers with expected attributes", function() {
      ...
      proclaim.equal("f0", flipBuffers[0].id, "first id is correct");
      proclaim.equal("f1", flipBuffers[1].id, "first id is correct");

      proclaim.equal(4, Array.from(theDiv.childNodes).length, "there are two new child Elements under theDiv");
    });
  });
});
