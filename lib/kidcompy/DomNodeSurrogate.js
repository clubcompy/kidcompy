 ...
 */
function DomNodeSurrogate(parent, nodeId, ownerDocument, nodeType, nodeCache, mutationsLog) {
  var self = this,
    domNode;
  ...
  /**
   * @name DomNodeSurrogate#ownerDocument
   * @type {!Document}
   */
  self.ownerDocument = ownerDocument;

  // Create a DOM Node for this DomNodeSurrogate, if appropriate
  domNode = self.createDom();
  ...
  if(domNode) {
    if(self.parent) {
      DomTools.addClass(domNode, self.nodeId);
    }

    self.attachNodeToParent(mutationsLog, domNode);
  }
}

/**
 * Generate DOM element for this DomNodeSurrogate.  Only create the DOM subtrees,
 *
 * <p>Override this function in implementations if a simple DIV
 * is not what you want for your DomNodeSurrogate subtree
 *
 * @returns {Element} root of your DomNodeSurrogate DOM subtree
 */
DomNodeSurrogate.prototype.createDom = function() {
  var self = this;

  // as a default action, only create a DIV tag for this DomNodeSurrogate if this DomNode should be a child
  // of a parent DomNodeSurrogate
  return self.parent && self.ownerDocument.createElement("div");
};

/**
 * Attach a DOM node to our parent DomNodeSurrogate.
 *
 * <p>Override this function if you need to drill into the
 * parent's DOM subtree to find an attach point, need any special logic executed on attach, or have
 * a completely different place in the DOM that you need to attach the DomNodeSurrogate
 *
 * @param {DomMutationsLog} domMutationsLog
 * @param {(Element|Node)} domNode
 */
DomNodeSurrogate.prototype.attachNodeToParent = function(domMutationsLog, domNode) {
  ...
  if(self.parent) {
    ...
    domMutationsLog.appendChild(parentDiv, domNode);
  }
};
