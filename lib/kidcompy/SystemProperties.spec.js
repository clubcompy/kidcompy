...
describe("SystemProperties.spec", function() {
  describe("System defaults", function() {
    ...
    it("allows for system defaults to be set in a static map", function() {
      ...
      proclaim.isFalse(systemProperties.hasProperty("propName"), "The property does not exist by default");

      // add a system default
      SystemProperties.setGlobalDefaultProperty("propName", "propValue");
      proclaim.isFalse(systemProperties.hasProperty("propName"), "Existing SystemProperties are not affected");

      // make a new system properties, see if the default is there
      systemProperties = new SystemProperties();
      proclaim.isTrue(systemProperties.hasProperty("propName"), "Property exists as default for new SystemProperties");
    });
    ...
    it("won't die if we construct a SystemProperties without any global defaults set", function() {
      ...
      proclaim.isNotNull(mySystemProperties);
      proclaim.equal(constants.DEFAULT_DISPLAY_WIDTH, mySystemProperties.getNumberProperty(constants.KEY_DISPLAY_WIDTH));

      kidcompy.globalPropertyDefaults = null;

      proclaim.doesNotThrow(function() {
        mySystemProperties = new SystemProperties();
      }, Error, "SystemProperties should be created even though there are no globalPropertyDefaults set");

      proclaim.isNotNull(mySystemProperties, "construction should have worked");
      proclaim.isFalse(mySystemProperties.hasProperty(constants.KEY_DISPLAY_WIDTH));
    });
  });
});
