...
Main.prototype.load = function() {
  ...
  iePolyfillInitSequence = self.checkIePolyfillsNeeded()
    .map(function(isIePolyfillNeeded) {
    ...
    .map(function() {
      ...
      return "ie polyfill script loaded";
    });

  html5PolyfillInitSequence = self.checkHtml5PolyfillsNeeded()
    .map(function(isHtml5PolyfillNeeded) {
    ...
    .map(function() {
      ...
      return "html5 polyfill script loaded";
    });

  // it's in here somewhere ...
  kidcompyScriptInitSequence = self.loadKidcompyScript()
    .map(function(kidcompyScript) {
      if(kidcompyScript) {
    ...
    .map(function() {
      ...
      return "kidcompy script loaded";
    });

  kidcompy.rxjs.Observable.forkJoin(iePolyfillInitSequence, html5PolyfillInitSequence, kidcompyScriptInitSequence)
    .subscribe(
      function(n) {
