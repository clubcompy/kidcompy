...
describe("KidcompyHost.spec", function() {
  ...
  describe("init", function() {
    ...
    it("adds FlipBuffers to theDiv", function() {
      ...
      proclaim.equal("f0", kidcompyHost.flipBuffers[0].classId);
      proclaim.equal("f1", kidcompyHost.flipBuffers[1].classId);
  ...
  describe("attachFlipBuffers", function() {
    it("constructs two FlipBuffers with expected attributes", function() {
      ...
      var flipBuffers = KidcompyHost.prototype.attachFlipBuffers(theDiv, new DomMutationsLog());

      proclaim.equal(2, flipBuffers.length, "there are 2 flipbuffers");
      proclaim.equal("f0", flipBuffers[0].classId, "first id is correct");
      proclaim.equal("f1", flipBuffers[1].classId, "first id is correct");
