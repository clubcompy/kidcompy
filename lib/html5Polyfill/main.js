...
/**
 * Make a closure for callback that calls callback with nextTime
 *
 * @param {!Function} callback
 * @param {!number} nextTime
 * @returns {!Function} callback caller
 */
function getPolyfilledRequestAnimationFrameCallback(callback, nextTime) {
  return function() {
    callback(nextTime);
  };
}
...
/**
 * Polyfill requestAnimationFrame if necessary
 */
function polyfillRequestAnimationFrame() {
  ...
  if(!window["requestAnimationFrame"]) {
    /**
     * setTimeout-based requestAnimationFrame polyfill
     *
     * @param {!Function} callback
    ...
    window["requestAnimationFrame"] = function(callback, element) {
      var currTime = kidcompy.now(),
        timeToCall = Math.max(0, 16 - (currTime - lastTime)),
        id = setTimeout(getPolyfilledRequestAnimationFrameCallback(callback, currTime + timeToCall), timeToCall);
 ...
 *
 * @public
 * @param {function(boolean)} done callback to be called once HTML5 polyfills init is complete
 */
