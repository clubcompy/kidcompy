 ...
 */
function FlipBuffer(kidcompyHost, nodeCache, ordinal, ownerDocument, systemProperties, mutationsLog) {
  var self = this;
  ...
  DomNodeSurrogate.call(self, kidcompyHost, "f" + ordinal, ownerDocument, DomNodeType.FLIPBUFFER, nodeCache,
    mutationsLog);
...
FlipBuffer.prototype.createPlayfields = function(nodeCache, mutationsLog) {
  ...
  for(i = 0, ii = 8; i < ii; i++) {
    plays.push(new Playfield(self, nodeCache, i, self.ownerDocument, self.systemProperties, mutationsLog));
  }
