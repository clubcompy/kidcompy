 ...
 * @param {!Array.<number>} pixelData array of integers containing 8bit character definition.  Each row of the character definition
 * is represented by a number in the pixelData string.  Starting from startOffset, 8 1-bit character definition
 ...
 * @returns {!Array.<number>} encoded sequences of 16-bit unsigned int left, top, width, height rectangle value groups
 */
  ...
  do {
    for(i = 0; i < bitsWideRowsTall; i++) {
      ...
      for(j = 0; j < bitsWideRowsTall; j++) {
        ...
        while(j < bitsWideRowsTall && row >> j & 1) {
        ...
        while(i < bitsWideRowsTall && (WORKING_BITS[i] & rowMask) === rowMask) {
        ...
        hei = i - sy;
        rectArray.push.call(rectArray, sx << scaleFactor, sy << scaleFactor, wid << scaleFactor,
          hei << scaleFactor);
 ...
 * @param {!Array.<number>} pixelData array containing 8bit character definition.  Each row of the character definition
 * is represented by a number in the pixelData array.  Starting from startOffset, 8 1-bit character definition
 * rows will be read from pixelData.
 * @returns {!Array.<number>} sequences of 16-bit unsigned int left, top, width, height rectangle value groups
 */
 ...
 * @param {!Array.<number>} pixelData array containing 8bit character definition.  Each row of the character definition
 * is represented by a number in the pixelData array.  Starting from startOffset, 16 1-bit character definition
 * rows will be read from pixelData.
 * @returns {!Array.<number>} sequences of 16-bit unsigned int left, top, width, height rectangle value groups
 */
