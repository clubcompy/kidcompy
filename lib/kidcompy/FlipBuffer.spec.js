...
describe("FlipBuffer.spec", function() {
  ...
  describe("init", function() {
    it("should add its div to the Host's div", sinon.test(function() {
      ...
      var flipBuffer = new FlipBuffer(fakeKidcompyHost, nodeCache, 0, window.document, new SystemProperties(),
        new DomMutationsLog());
    ...
    it("must add 8 Playfields to the FlipBuffer", function() {
      var flipBuffer = new FlipBuffer(fakeKidcompyHost, nodeCache, 1, window.document, new SystemProperties(),
        new DomMutationsLog());
