 ...
 */
function generateRectangleListFromCharacterDefinition(startOffset, pixelData, bitsWideRowsTall) {
 ...
 *
 * @param {!number} startOffset starting index into pixelData.  Note: this code assumes that startOffset+8 <= pixelData.length!
 * @param {!string} pixelData string containing 8bit character definition.  Each row of the character definition
 * is represented by a character in the pixelData string.  Starting from startOffset, 8 1-bit character definition
 * rows will be read from pixelData.
 * @returns {!string} encoded sequences of 16-bit unsigned int left, top, width, height rectangle value groups
...
CharacterGenerator.prototype.rectanglesFrom8x8 = function(startOffset, pixelData) {
  return generateRectangleListFromCharacterDefinition(startOffset, pixelData, 8);
};
 ...
 * if we wind up with a lot of little ones
 *
 * @param {!number} startOffset starting index into pixelData.  Note: this code assumes that startOffset+16 <= pixelData.length!
 * @param {!string} pixelData string containing 8bit character definition.  Each row of the character definition
 * is represented by a character in the pixelData string.  Starting from startOffset, 16 1-bit character definition
 * rows will be read from pixelData.
 * @returns {!string} encoded sequences of 16-bit unsigned int left, top, width, height rectangle value groups
 */
CharacterGenerator.prototype.rectanglesFrom16x16 = function(startOffset, pixelData) {
  return generateRectangleListFromCharacterDefinition(startOffset, pixelData, 16);
};
