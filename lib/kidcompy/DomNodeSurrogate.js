...
function DomNodeSurrogate(parent, nodeId, ownerDocument, nodeType, nodeCache, mutationsLog) {
  ...
  if(self.parent) {
    childDiv = self.createDom();
    DomTools.addClass(childDiv, self.nodeId);

    self.attachNodeToParent(mutationsLog, childDiv);
  }
}
...
DomNodeSurrogate.prototype.createDom = function() {
  ...
  return self.ownerDocument.createElement("div");
};
 ...
 * parent's DOM subtree to find an attach point or need any special logic executed on attach.
 *
